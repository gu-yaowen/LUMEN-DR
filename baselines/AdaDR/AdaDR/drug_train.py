...
if __name__ == '__main__':
    ...
    aucs, auprs = [], []
    all_true, all_score = [], []
    # 5-fold cross validation (LMDDA compatible)
    for times in range(1, 2):  
        ...
        n_folds = len(dataset.data_cv)
        for cv in range(0, n_folds):
            args.save_id = cv + 1
            print("===============" + str(cv + 1) + "=================")
            graph_data = dataset.data_cv[cv]
            model = train(args, dataset, graph_data, cv)
            ...
            # Save per-fold predictions with drug_id and disease_id
            test_data_info = dataset.cv_data_dict[cv][1]  # test DataFrame
            fold_predictions = []
            for idx in range(len(y_true)):
                fold_predictions.append({
                    'drug_id': int(test_data_info.iloc[idx]['drug_id']),
                    'disease_id': int(test_data_info.iloc[idx]['disease_id']),
                    'true_label': int(y_true[idx]),
                    'pred_prob': float(y_score[idx])
                })
            pred_file = f'../adadr_predictions_{args.data_name}_fold{cv + 1}.json'
            with open(pred_file, 'w') as f:
                json.dump(fold_predictions, f)
            print(f'Fold {cv + 1} predictions saved to {pred_file}')
    
    # Pooled metrics
    from sklearn import metrics as skm
    pooled_auroc = skm.roc_auc_score(all_true, all_score)
    pooled_aupr = skm.average_precision_score(all_true, all_score)
    print(f"\n{'='*60}")
    print(f"AdaDR on {args.data_name} - Pooled Results ({n_folds}-fold CV)")
    print(f"  AUROC: {pooled_auroc:.4f}")
    print(f"  AUPR:  {pooled_aupr:.4f}")
    print(f"  Per-fold AUROC: {[f'{a:.4f}' for a in aucs]}")
    print(f"  Per-fold AUPR:  {[f'{a:.4f}' for a in auprs]}")
    print(f"{'='*60}")
    
    results = {'model': 'AdaDR', 'dataset': args.data_name,
               'auroc': pooled_auroc, 'aupr': pooled_aupr,
               'fold_aurocs': aucs, 'fold_auprs': auprs}
    with open(f'../formal_results_adadr_{args.data_name}.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to ../formal_results_adadr_{args.data_name}.json")
    ...
    pooled_pred_file = f'../adadr_predictions_{args.data_name}_pooled.json'
